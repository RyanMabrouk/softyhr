...
function Description({ job }: DescriptionPropsType) {
  ...
  return (
    <div className="w-full py-4">
      <div className="flex flex-col items-start justify-center gap-[1rem] pb-8">
        ...
        <h1 className="text-sm text-gray-15">
          {job?.job_information?.["Departement"] +
            " · " +
            job?.job_information?.["Job Location"]}
        </h1>
      </div>
      <div className="border-t border-gray-32 py-4" ref={TextRef}></div>
    </div>
  );
}
