...
export function History() {
  ...
  const {
    ...
    all_profiles: { data: all_profiles, isPending: isPending3 },
  } = useData();
  const {
    leave_requests: { data: leave_requests, isPending: isPending5 },
    leave_accrued: { data: leave_accrued, isPending: isPending6 },
    employee_profile: { data: employee_profile, isPending: isPending7 },
  ...
  const leave_requests_data: leave_data_types = leave_requests
    ...
    .map((e: database_leave_requests_type) => {
      ...
      const {
        employee_profile: { data: reviewed_by_info },
      } = useEmployeeData({
        employeeId: e.reviewed_by,
      })?.["Basic Information"];
      return {
