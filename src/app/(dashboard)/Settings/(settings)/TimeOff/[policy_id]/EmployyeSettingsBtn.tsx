...
export function EmployyeSettingsBtn({ employeeId }: { employeeId: string }) {
  ...
  return (
    ...
    >
      <div
        id="timeoff_settings"
        className={`flex cursor-pointer bg-white flex-row items-center justify-center gap-1 rounded-md border border-gray-25 px-2 py-1.5 shadow-sm transition-all ease-linear hover:shadow-md group-hover:opacity-100 ${open ? "opacity-100" : "opacity-0"}`}
      >
