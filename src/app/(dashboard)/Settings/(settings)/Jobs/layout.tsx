...
function Layout({ children }: { children: React.ReactNode }) {
  ...
  const JobSettingsRoutes: JobSettingsRoutesType[] = [
    { label: "Candidate Statuses", path: "/" },
    { label: "Candidate Sources", path: "/CandidateSources" },
            ...
            return (
              <div key={"categories" + index} className="flex flex-col">
                <Link
                  ...
                  className={` rounded-sm p-2 ${(pathname?.includes(Route?.path) && Route?.label != JobSettingsRoutes[0]?.label) || (pathname == "/Settings/Jobs" && Route?.label == JobSettingsRoutes[0]?.label) ? "bg-gray-14 font-bold text-fabric-700" : ""} text-[0.95rem] font-normal  transition-all ease-linear hover:bg-gray-14`}
                >
                  {Route?.label}
                </Link>
              </div>
