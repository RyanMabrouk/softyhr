...
function InboxLink({
  ...
  setToggleView,
  setNotifs,
  dataSetName, // the name of the object that has the data returned from the hook
}: {
  label: string;
  path: string;
  ...
  setNotifs: React.Dispatch<React.SetStateAction<number>>;
}) {
  const {
    [dataSetName]: { data: requests },
  } = hook();
  useEffect(() => {
    setNotifs((old) => old + (requests?.length ?? 0));
  }, [requests?.length, setNotifs]);
  return (
    <Link
      onClick={() => setToggleView((old) => !old)}
